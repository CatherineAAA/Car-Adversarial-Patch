...
class PatchTrainer(object):
    def __init__(self, mode):
        ...
        self.prob_extractor = MaxProbExtractor(2, 17, 80, self.config).cuda()
    ...
    def train(self):
        ...
        adv_patch_cpu = self.read_image("./car_patch/bbox_nowarp/20191213-161949_base_85.png")
        ...
        for epoch in range(n_epochs):
            ...
            for i_batch, (images, datadirs) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}',
               ...
               with autograd.detect_anomaly():
                    ...
                    mask = [gt[:, 0].cpu().numpy() - 1/10. * mask_w, gt[:, 1].cpu().numpy() - 1/10. * mask_h, gt[:, 2].cpu().numpy() + 1/10. * mask_w, gt[:, 3].cpu().numpy() + 1/10. * mask_h]
                    mask = torch.Tensor(np.array(mask).transpose((1, 0)))
                    image_mask = torch.ones_like(images).cuda() * 0.5
                    for gt_idx in range(len(mask)):
                        image_mask[:, :, int(mask[gt_idx][1].item()):int(mask[gt_idx][3].item()), int(mask[gt_idx][0].item()):int(mask[gt_idx][2].item())] = 1
                    ...
                    # warp patch
                    # tmp_img = np.array(adv_patch.detach().cpu()).transpose((1, 2, 0)) * 255
                    # tmp_ih, tmp_iw, _ = tmp_img.shape

                    # tmp_black = np.zeros((int(tmp_ih), int(tmp_iw), 3), np.uint8)
                    # pts_src = np.array([[0.0, 0.0], [float(tmp_iw), 0.0], [float(tmp_iw), float(tmp_ih)], [0.0,float(tmp_ih)]])
                    # pts_dst = np.array([[self.deformation, 0], [self.patch_width - self.deformation, 0], [tmp_iw, tmp_ih], [0, tmp_ih]])
                    # tmp_h, status = cv2.findHomography(pts_src, pts_dst)
                    # tmp_im_out = cv2.warpPerspective(tmp_img, tmp_h, (tmp_black.shape[1],tmp_black.shape[0]))
                    # tmp_img_tensor = torch.Tensor((tmp_im_out / 255.).transpose((2, 0, 1)))

                    # warped_patch = self.warp_patch.main(tmp_img_tensor, self.patch_width, self.patch_size)
                    # adv_patch.data = warped_patch.data
                    ...
                    img.save("/data5/gepei/adversarial_example/car_patch/car_bbox_nowarp.png")
                    ...
                    loss = bbox_loss +  0.01 * nps + torch.max(tv, torch.tensor(0.1).cuda())
                    # loss = predict_loss +  0.01 * nps + torch.max(tv, torch.tensor(0.1).cuda())
                    # loss = 0.5 * predict_loss + 0.5 * bbox_loss +  0.01 * nps + torch.max(tv, torch.tensor(0.1).cuda())  
            ...
            im.save(f'/data5/gepei/adversarial_example/car_patch/bbox_nowarp/{time_str}_{self.config.patch_name}_{epoch}.png')
